...
# Uso: INSTALL_DIR=/opt/precisoutapronto bash vultr-deploy-precisoutapronto.sh
...
for state_file in .indexnow-sitemap.xml .indexnow-updated-urls.sha256; do
  if [[ -f "${INSTALL_DIR}/${state_file}" ]]; then
    cp -a "${INSTALL_DIR}/${state_file}" "/tmp/precisoutapronto.${state_file}.bak"
  fi
done
find "${INSTALL_DIR}" -mindepth 1 -maxdepth 1 ! -name '.env.production' -exec rm -rf {} +
tar -xzf "${TARBALL}" -C "${INSTALL_DIR}"
if [[ ! -f "${INSTALL_DIR}/.env.production" && -f /tmp/precisoutapronto.env.production.bak ]]; then
  cp -a /tmp/precisoutapronto.env.production.bak "${INSTALL_DIR}/.env.production"
  chmod 600 "${INSTALL_DIR}/.env.production"
fi
for state_file in .indexnow-sitemap.xml .indexnow-updated-urls.sha256; do
  if [[ ! -f "${INSTALL_DIR}/${state_file}" && -f "/tmp/precisoutapronto.${state_file}.bak" ]]; then
    cp -a "/tmp/precisoutapronto.${state_file}.bak" "${INSTALL_DIR}/${state_file}"
  fi
done
