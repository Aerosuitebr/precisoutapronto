...
export async function generateMetadata({ searchParams }: { searchParams: Promise<Search> }): Promise<Metadata> {
  ...
  const title = one(search.titulo, 'Resultado Pronto compartilhado');
  ...
  const description = `${label}: ${value}. Veja o resumo e crie o seu gratuitamente no Precisou, Tá Pronto.`;
  return {
    title: { absolute: `${title} | Precisou, Tá Pronto` },
    description,
...
export default async function PublicResultPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const campaign = one(search.utm_campaign, 'resultado_pronto', 100);
  ...
  return <><SiteHeader /><main className="min-h-[70vh] bg-[linear-gradient(160deg,#020617,#082f49_55%,#0c4a6e)] px-4 py-14 text-white"><div className="mx-auto max-w-2xl"><div className="flex items-center gap-2 text-sm font-bold uppercase tracking-[0.18em] text-sky-300"><Sparkles className="h-4 w-4" />Resultado Pronto</div><section className="mt-5 rounded-[28px] border border-white/15 bg-white/10 p-6 shadow-2xl backdrop-blur sm:p-8"><h1 className="precisoutapronto-display text-3xl font-extrabold sm:text-4xl">{title}</h1>{lines.length ? <dl className="mt-7 divide-y divide-white/10 rounded-2xl bg-slate-950/35 px-5">{lines.map((line, index) => <div key={`${line.label}-${index}`} className="flex items-center justify-between gap-4 py-4"><dt className="text-sm text-slate-300">{line.label}</dt><dd className="text-right font-bold">{line.value}</dd></div>)}</dl> : null}<div className="mt-6 flex items-center justify-between gap-4 rounded-2xl bg-slate-950 px-5 py-5"><span className="text-sm font-semibold text-slate-300">{label}</span><strong className="precisoutapronto-display text-2xl text-emerald-300">{value}</strong></div><p className="mt-6 text-center text-xs text-slate-300">Resultado compartilhado por alguém usando o Precisou, Tá Pronto.</p></section><PublicResultActions title={title} value={value} toolPath={toolPath} campaign={campaign} /></div></main><SiteFooter /></>;
}
