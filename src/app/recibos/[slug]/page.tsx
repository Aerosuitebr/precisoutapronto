...
export default async function ReceiptClusterRoute({ params }: Props) {
  ...
  const updatedAt = page.updatedAt ?? '2026-09-05';
  const related = page.related.map(getReceiptClusterPage).filter((item): item is NonNullable<typeof item> => Boolean(item));
  const jsonLd = { '@context': 'https://schema.org', '@graph': [
    { '@type': 'Article', headline: page.title, description: page.description, inLanguage: 'pt-BR', datePublished: '2026-08-10', dateModified: updatedAt, mainEntityOfPage: `${base}/recibos/${page.slug}`, author: { '@type': 'Organization', name: 'Equipe editorial Precisou, Tá Pronto', url: `${base}${BRAND_AUTHOR_PATH}` }, publisher: { '@type': 'Organization', name: 'Precisou, Tá Pronto', url: base } },
