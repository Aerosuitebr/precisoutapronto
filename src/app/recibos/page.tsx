...
export const metadata: Metadata = {
  ...
  description:
    'Gere recibo de Pix, serviço, autônomo ou aluguel. O comprovante do banco mostra a transferência. O recibo explica o que foi quitado. PDF grátis, sem cadastro.',
  alternates: { canonical: '/recibos' },
  openGraph: {
    title: 'Gerador de recibo Pix e modelos em PDF',
    description: 'Crie o recibo depois de confirmar o crédito. Não imita extrato bancário.',
    url: '/recibos',
    type: 'website'
  }
};
...
const hubFaqs = [
  {
    question: 'Como gerar um recibo de Pix online?',
    answer:
      'Confirme o crédito no seu banco, abra o gerador, informe pagador, recebedor, valor, data e o que aquele Pix quitou. Marque pago via Pix e baixe o PDF para enviar no WhatsApp.'
  },
  {
    question: 'O comprovante do Pix serve como recibo?',
    answer:
      'Não por si só. O comprovante prova que o dinheiro saiu de uma conta para outra. O recibo registra a finalidade: serviço, aluguel, sinal ou parcela. Use os dois juntos.'
  },
  {
    question: 'Gerador de recibo Pix cria comprovante bancário?',
    answer:
      'Não. Este gerador emite um recibo com a obrigação quitada. Não imita extrato de banco. Não use arquivo de “comprovante gerado” para fingir um Pix que ainda não caiu na conta.'
  },
  {
...
export default function ReceiptHubPage() {
  ...
  const jsonLd = {
    ...
    '@graph': [
      {
        '@type': 'CollectionPage',
        name: 'Gerador de recibo Pix e modelos em PDF',
      ...
      <main>
        <section className="bg-slate-950 text-white">
          <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 sm:py-20">
            ...
            <h1 className="precisoutapronto-display mt-4 max-w-3xl text-4xl font-extrabold tracking-tight sm:text-5xl">
              Gerador de recibo Pix e modelos para cada pagamento.
            </h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-200">
              O comprovante do banco mostra a transferência. O recibo explica o que foi quitado. Preencha online e
        ...
        <section className="mx-auto max-w-5xl px-4 py-14 sm:px-6">
          ...
          <h2 className="precisoutapronto-display mt-14 text-3xl font-extrabold text-slate-950">
            Tipos, formatos e dúvidas sobre recibo
          </h2>
          <div className="mt-7 grid gap-4 sm:grid-cols-2">
            {receiptClusterPages.map((page) => (
              <Link
                key={page.slug}
                href={`/recibos/${page.slug}`}
                className="group rounded-2xl border border-slate-200 p-5 transition hover:border-sky-300 hover:bg-sky-50/40"
              >
                <h3 className="font-bold leading-6 text-slate-950 group-hover:text-sky-800">{page.title}</h3>
        ...
        <section className="border-y border-slate-200 bg-emerald-50">
          <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
            <h2 className="precisoutapronto-display text-3xl font-extrabold text-slate-950">Um único gerador, vários casos comuns</h2>
            <ul className="mt-6 grid gap-3 sm:grid-cols-3">
              {['Valor por extenso automático', 'Modelos profissionais em PDF', 'Assinatura e histórico'].map((item) => (
                <li key={item} className="flex items-center gap-2 rounded-xl bg-white p-4 text-sm font-semibold text-slate-800">
        ...
        <section className="mx-auto max-w-5xl px-4 py-14 sm:px-6">
          <h2 className="precisoutapronto-display text-3xl font-extrabold text-slate-950">Perguntas frequentes</h2>
        ...
        <StrategicSeoClusters current="/recibos/recibo-pagamento-pix" />
      </main>
      <SiteFooter />
    </>
  );
