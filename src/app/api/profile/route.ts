...
export async function PUT(request: Request) {
  ...
  const body = await request.json().catch(() => ({})) as { segment?: string; companyName?: string; occupation?: string };
  ...
  const profile = await getPrisma().userProfile.upsert({
    ...
    create: { userId: session.sub, segment: body.segment, companyName: body.companyName?.slice(0, 120), occupation: body.occupation?.slice(0, 120) },
    update: { segment: body.segment, companyName: body.companyName?.slice(0, 120), occupation: body.occupation?.slice(0, 120) }
  });
  return NextResponse.json({ profile });
}
