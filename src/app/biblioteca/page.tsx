      ...
      <main className="bg-slate-50">
        ...
        <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {intentPages.map((item) => <Link key={item.slug} href={`/modelos/${item.slug}`} className="rounded-2xl border border-slate-200 bg-white p-5 hover:border-emerald-300"><h3 className="font-bold text-slate-950">{item.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{item.description}</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-emerald-700">Ver resposta <ChevronRight className="h-4 w-4" /></span></Link>)}
          </div>
