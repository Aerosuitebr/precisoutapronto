...
const professionLinks = [
  ...
  { href: '/orcamento-para/chaveiro', label: 'Chaveiro' },
  { href: '/orcamento-para/marceneiro', label: 'Marceneiro' },
  { href: '/orcamento-para/gesseiro', label: 'Gesseiro' },
  { href: '/orcamento-para/instalacao-de-piso', label: 'Instalação de piso' }
] as const;
...
export function LandingPage() {
  return <div className="bg-white text-[#101828]">
    ...
    <main>
        ...
        <div className="mx-auto grid max-w-7xl gap-14 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[1.02fr_.98fr] lg:items-center lg:px-8 lg:py-28">
          ...
          <div className="relative mx-auto w-full max-w-xl lg:mx-0">
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-3 shadow-[0_32px_80px_-24px_rgba(16,24,40,.28)] sm:p-5"><div className="rounded-[1.5rem] border border-slate-100 bg-white p-5 sm:p-7">
              ...
              <div className="flex items-end justify-between gap-4 border-t border-slate-100 pt-5"><div><p className="text-[10px] font-black uppercase tracking-widest text-slate-600">Total</p><p className="mt-1 text-4xl font-black tracking-[-.05em]">R$ 490</p></div><div className="flex items-center gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700"><QrCode className="h-5 w-5" /> Pix pronto</div></div>
              <div className="mt-6 grid grid-cols-2 gap-3"><span className="rounded-xl bg-emerald-700 px-3 py-3.5 text-center text-sm font-black text-white">Aprovar orçamento</span><span className="rounded-xl border border-slate-200 px-3 py-3.5 text-center text-sm font-black text-slate-700">Pedir ajuste</span></div>
            </div></div>
      ...
      <section className="border-b border-slate-200 py-8"><div className="mx-auto flex max-w-7xl flex-col items-center gap-5 px-4 sm:px-6 lg:flex-row lg:justify-between lg:px-8"><p className="text-xs font-black uppercase tracking-[.16em] text-slate-600">Para quem presta serviço</p><ul className="flex flex-wrap justify-center gap-x-7 gap-y-3 text-sm font-black text-slate-700">{['Eletricistas', 'Pintores', 'Instaladores', 'Freelancers', 'Manutenção'].map(item => <li key={item}>{item}</li>)}</ul></div></section>

      <section className="py-20 sm:py-28"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[.18em] text-[#155eef]">Um fluxo. Sem enrolação.</p><h2 className="precisoutapronto-display mt-4 text-4xl font-black tracking-[-.05em] sm:text-6xl">Da conversa ao pagamento.</h2><p className="mt-5 text-lg leading-8 text-slate-600">Pare de procurar mensagem, somar valor na calculadora e mandar o Pix separado.</p></div><ol className="mt-12 grid gap-5 lg:grid-cols-3">{steps.map(step => <li key={step.number} className="group rounded-[1.75rem] border border-slate-200 bg-[#f8faff] p-7 transition hover:-translate-y-1 hover:border-blue-200 hover:bg-white hover:shadow-xl"><div className="flex items-center justify-between"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#155eef] text-white"><step.icon className="h-6 w-6" /></span><span className="text-4xl font-black text-slate-500">{step.number}</span></div><h3 className="mt-8 text-2xl font-black">{step.title}</h3><p className="mt-3 text-base leading-7 text-slate-600">{step.text}</p></li>)}</ol></div></section>

      <section aria-labelledby="documentos-servico-title" className="border-y border-slate-200 bg-slate-50 py-14 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 id="documentos-servico-title" className="precisoutapronto-display text-3xl font-black tracking-[-.04em] sm:text-4xl">O documento certo para cada etapa do serviço</h2>
          <p className="mt-4 max-w-2xl leading-7 text-slate-600">Prepare a proposta, combine o orçamento e registre o pagamento. Escolha o que você precisa agora.</p>
          <ul className="mt-8 grid gap-4 lg:grid-cols-3">
            {commercialTools.map(tool => <li key={tool.href}>
              <LandingConversionLink href={tool.href} landingPath="/" placement="inline_primary" className="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-blue-300 hover:shadow-lg">
                <h3 className="text-xl font-black">{tool.title}</h3>
                <p className="mt-3 flex-1 text-sm leading-7 text-slate-600">{tool.description}</p>
                <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#155eef]">{tool.action}<ArrowRight aria-hidden="true" className="h-4 w-4 shrink-0" /></span>
              </LandingConversionLink>
            </li>)}
          </ul>
          <h3 className="mt-10 text-lg font-bold">Orçamento para a sua profissão</h3>
          <ul className="mt-4 flex flex-wrap gap-3">
            {professionLinks.map(item => <li key={item.href}><LandingConversionLink href={item.href} landingPath="/" placement="inline_primary" className="inline-flex min-h-11 items-center rounded-xl border border-blue-200 bg-white px-4 py-2 text-sm font-bold text-[#155eef] hover:bg-blue-50">{item.label}</LandingConversionLink></li>)}
          </ul>
          <Link href="/modelos-de-orcamento" className="mt-5 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-[#155eef]">Ver todos os modelos de orçamento <ArrowRight aria-hidden="true" className="h-4 w-4" /></Link>
        </div>
      </section>
