...
export function LandingPage() {
  return (
    <div className="bg-[#f7f8f5] text-slate-950">
      ...
      <section className="border-b border-slate-200 bg-[#f7f8f5]"><div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[0.72fr_1.28fr] lg:items-start"><div className="lg:sticky lg:top-28"><p className="text-xs font-black uppercase tracking-[0.2em] text-sky-700">Em destaque agora</p><h2 className="rj-display mt-3 text-4xl font-black tracking-[-0.035em]">Outras tarefas que estão resolvendo o dia.</h2><p className="mt-4 text-sm leading-7 text-slate-600">Sugestões diferentes dos atalhos do topo, para ampliar sua descoberta sem repetir caminhos.</p></div><ul className="divide-y divide-slate-200 border-y border-slate-200">{popularNow.map((item, index) => <li key={item.href}><Link href={item.href} className="group grid grid-cols-[auto_1fr_auto] items-center gap-4 py-5 sm:gap-6"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white text-emerald-800 ring-1 ring-slate-200"><item.icon className="h-4 w-4" /></span><span><strong className="block text-lg group-hover:text-emerald-800">{item.label}</strong><span className="mt-1 block text-sm text-slate-500">{item.note}</span></span><ArrowRight className="h-5 w-5 text-slate-300 transition group-hover:translate-x-1 group-hover:text-emerald-700" /></Link></li>)}</ul></div></section>

      <section id="catalogo" className="scroll-mt-24 border-b border-slate-200 bg-white"><div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24"><div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">Catálogo por área</p><h2 className="rj-display mt-3 text-4xl font-black tracking-[-0.035em] sm:text-5xl">Filtre pelo tipo de resultado que precisa.</h2><p className="mt-5 text-base leading-8 text-slate-600">Finanças, documentos, carreira, PDFs e outras tarefas organizadas para você comparar opções sem percorrer uma lista interminável.</p></div><div className="mt-10"><CategoryExplorer /></div></div></section>

      <section className="relative bg-slate-950 text-white before:absolute before:inset-x-0 before:top-0 before:h-20 before:-translate-y-full before:bg-gradient-to-b before:from-transparent before:to-slate-950/10"><div className="mx-auto grid max-w-6xl gap-12 px-4 py-20 sm:px-6 sm:py-28 lg:grid-cols-[1fr_0.8fr] lg:items-center"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-300">Confiança antes da ação</p><h2 className="rj-display mt-3 max-w-2xl text-4xl font-black tracking-[-0.035em] sm:text-5xl">Menos promessa. Mais clareza antes de clicar.</h2><p className="mt-5 max-w-xl text-base leading-8 text-slate-300">Você vê o que a ferramenta faz, quais dados precisa informar e como será o resultado. Sem texto inflado para esconder uma experiência vazia.</p><Link href="/qualidade-e-seguranca" className="mt-7 inline-flex items-center gap-2 text-sm font-extrabold text-amber-300">Conhecer nossos critérios <ArrowRight className="h-4 w-4" /></Link></div><div className="grid gap-3">{[{ icon: Search, title: 'Encontre pela tarefa', text: 'Busque como você falaria, sem decorar nomes.' }, { icon: ShieldCheck, title: 'Dados sob controle', text: 'Ferramentas locais informam quando o arquivo não sai do navegador.' }, { icon: BadgeCheck, title: 'Resultado verificável', text: 'Exemplos, memória de cálculo e documentos visualizados antes de baixar.' }].map((item) => <div key={item.title} className="flex gap-4 rounded-2xl border border-white/10 bg-white/5 p-5"><item.icon className="mt-0.5 h-5 w-5 shrink-0 text-emerald-300" /><div><h3 className="font-extrabold">{item.title}</h3><p className="mt-1 text-sm leading-6 text-slate-400">{item.text}</p></div></div>)}</div></div></section>
    </div>
  );
}
