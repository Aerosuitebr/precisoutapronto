...
export function ViralClusterPage({ cluster }: { cluster: ViralCluster }) {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><SiteHeader /><main>
    <section className="bg-slate-950 text-white"><div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 sm:py-20"><nav className="flex items-center gap-1 text-xs text-slate-400"><Link href="/">Início</Link><ChevronRight className="h-3 w-3" />{cluster.eyebrow}</nav><p className="mt-7 text-sm font-bold uppercase tracking-[0.18em] text-emerald-300">{cluster.eyebrow}</p><h1 className="precisoutapronto-display mt-4 max-w-3xl text-4xl font-extrabold leading-tight sm:text-5xl">{cluster.h1}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-slate-200">{cluster.answer}</p><Link href={cluster.primary.href} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-amber-400 px-6 py-3 font-bold text-slate-950">{cluster.primary.label}<ArrowRight className="h-4 w-4" /></Link></div></section>
    ...
    <section className="mx-auto max-w-5xl px-4 py-14 sm:px-6"><h2 className="precisoutapronto-display text-3xl font-extrabold text-slate-950">Escolha pela sua intenção</h2><div className="mt-7 grid gap-4 sm:grid-cols-2">{cluster.resources.map((resource) => <Link key={resource.href} href={resource.href} className="group rounded-2xl border border-slate-200 p-5 hover:border-emerald-300"><p className="text-xs font-bold uppercase tracking-[0.12em] text-emerald-700">{resource.intent}</p><h3 className="mt-2 text-lg font-bold text-slate-950 group-hover:text-emerald-800">{resource.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{resource.description}</p></Link>)}</div>
    {cluster.sections.map((section) => <section key={section.title} className="mt-12"><h2 className="precisoutapronto-display text-2xl font-bold text-slate-950">{section.title}</h2><p className="mt-4 max-w-3xl leading-8 text-slate-700">{section.body}</p></section>)}
    <section className="mt-12"><h2 className="precisoutapronto-display text-2xl font-bold text-slate-950">Perguntas frequentes</h2><div className="mt-5 divide-y divide-slate-200 rounded-2xl border border-slate-200">{cluster.faqs.map((faq) => <div key={faq.question} className="p-5"><h3 className="font-bold text-slate-950">{faq.question}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{faq.answer}</p></div>)}</div></section></section>
  </main><SiteFooter /></>;
}
