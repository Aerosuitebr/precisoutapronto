    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-16">
        ...
        <article className="mt-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
          ...
          <h1 className="precisoutapronto-display mt-3 text-3xl font-extrabold text-slate-950 sm:text-4xl">{t.title}</h1>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
        <div className="mx-auto max-w-2xl text-center"><p className="text-sm font-bold uppercase tracking-[0.18em] text-sky-700">{t.subtitle}</p><h1 className="precisoutapronto-display mt-3 text-4xl font-extrabold">{t.title}</h1><p className="mt-4 leading-7 text-slate-600">{t.description}</p></div>
...
function PlanCard({ title, note, price, items, dark = false, children }: { title: string; note: string; price: string; items: readonly string[]; dark?: boolean; children: React.ReactNode }) {
  return <article className={`rounded-[28px] border p-8 shadow-sm ${dark ? 'border-slate-800 bg-slate-950 text-white' : 'border-slate-200 bg-white text-slate-950'}`}><h2 className="text-2xl font-extrabold">{title}</h2><p className={`mt-1 text-sm ${dark ? 'text-slate-300' : 'text-slate-600'}`}>{note}</p><p className="precisoutapronto-display mt-6 text-4xl font-extrabold">{price}</p><ul className="my-7 space-y-3 text-sm">{items.map((item) => <li key={item} className="flex gap-2"><Check className={`mt-0.5 h-4 w-4 shrink-0 ${dark ? 'text-amber-300' : 'text-sky-600'}`} />{item}</li>)}</ul>{children}</article>;
}
