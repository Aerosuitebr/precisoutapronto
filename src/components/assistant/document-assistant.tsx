...
const flows: Record<AssistantType, { label: string; editor: string; questions: Array<{ key: string; label: string; placeholder: string }> }> = {
  contrato: { label: 'Contrato de serviço', editor: '/ferramentas/contratos', questions: [
  ...
  curriculo: { label: 'Currículo', editor: '/ferramentas/curriculo', questions: [
  ...
  recibo: { label: 'Recibo', editor: '/ferramentas/recibos', questions: [
  ...
  proposta: { label: 'Proposta comercial', editor: '/ferramentas/propostas', questions: [
...
export function DocumentAssistant({ initialType = 'contrato' }: { initialType?: AssistantType }) {
  ...
  return <div className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-8">
    <div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-emerald-100 text-emerald-700"><Bot /></span><div><p className="font-bold text-slate-950">Assistente Precisou, Tá Pronto</p><p className="text-sm text-slate-500">Organiza o caso antes de abrir o editor atual</p></div></div>
