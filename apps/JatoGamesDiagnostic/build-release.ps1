...
$exePath = Join-Path $Output 'JatoGamesDiagnostic.exe'
if (-not (Test-Path -LiteralPath $exePath)) {
  throw "Executável não encontrado em $exePath"
}

$hash = Get-FileHash -Algorithm SHA256 -LiteralPath $exePath
$hashLine = "$($hash.Hash.ToLowerInvariant())  JatoGamesDiagnostic.exe"
Set-Content -LiteralPath (Join-Path $Output 'JatoGamesDiagnostic.sha256') -Value $hashLine -Encoding ascii
