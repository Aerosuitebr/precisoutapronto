...
public partial class MainWindow : Window
{
    ...
    private async void StartButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show("O teste foi cancelado com segurança. Nenhum resultado foi enviado.", "Precisou, Tá Pronto Games Diagnostic", MessageBoxButton.OK, MessageBoxImage.Information);
            Restart();
        }
    ...
    private void ExportButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            Title = "Exportar diagnóstico Precisou, Tá Pronto Games",
            Filter = "Relatório JSON (*.json)|*.json",
            FileName = $"precisoutapronto-games-diagnostic-{DateTime.Now:yyyyMMdd-HHmm}.json"
        };
        if (dialog.ShowDialog(this) == true)
        {
            try
            {
                ...
                MessageBox.Show("Relatório técnico exportado sem nome do computador ou usuário. Compartilhe somente se desejar.", "Precisou, Tá Pronto Games Diagnostic", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception exception)
            {
                LocalLog.Write(exception, "Exportação do relatório");
                MessageBox.Show("Não foi possível salvar nesse local. Escolha uma pasta em que você tenha permissão.", "Precisou, Tá Pronto Games Diagnostic", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }
    }
    ...
    private async Task RefreshCatalogAsync(bool notifyFailure)
    {
        ...
        CatalogStatus.Text = "Verificando Top 10 do Precisou, Tá Pronto Games…";
        try
        {
        ...
        catch
        {
            ...
            if (notifyFailure)
                MessageBox.Show("Não foi possível consultar o catálogo agora. A lista offline continua disponível.", "Precisou, Tá Pronto Games Diagnostic", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        finally
        {
