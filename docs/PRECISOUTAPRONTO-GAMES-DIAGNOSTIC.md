...
O `PrecisouTaProntoGamesDiagnostic` é um aplicativo Windows complementar ao Precisou, Tá Pronto Games. Ele coleta inventário real da máquina, executa benchmarks locais e gera um relatório comparativo para o jogo selecionado.

A candidata 0.9.0 está em `apps/PrecisouTaProntoGamesDiagnostic` e usa .NET 10 LTS.
...
```powershell
powershell -ExecutionPolicy Bypass -File apps/PrecisouTaProntoGamesDiagnostic/build-release.ps1
```

Saída:

- `dist/PrecisouTaProntoGamesDiagnostic/PrecisouTaProntoGamesDiagnostic.exe`
- `dist/PrecisouTaProntoGamesDiagnostic/PrecisouTaProntoGamesDiagnostic.sha256`
