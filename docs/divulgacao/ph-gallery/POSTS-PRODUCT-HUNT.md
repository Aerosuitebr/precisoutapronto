# Product Hunt · pack visual rico + posts

Visual no mesmo padrão das artes de hoje (navy / gold / sky, cinematográfico).

## Arquivos (suba na gallery / thumbnail)

Pasta: `docs/divulgacao/ph-gallery/`

| Ordem sugerida na gallery | Arquivo | Uso |
|---|---|---|
| 1 (hero) | `ph-gallery-marca.png` | Visão geral da marca |
| 2 | `ph-gallery-orcamento-pix.png` | Orçamento + Pix |
| 3 | `ph-gallery-curriculo.png` | Currículo |
| 4 | `ph-gallery-recibo.png` | Recibo |
| 5 | `ph-gallery-proposta.png` | Proposta comercial |
| Thumbnail | `ph-thumb-rj.png` | Ícone / thumb do launch |

Também serve recortar as stories de `docs/divulgacao/assets/promo-*-stories.png` se quiser vertical no social.

Link canônico: `https://resolvajato.com.br/?utm_source=producthunt&utm_medium=referral&utm_campaign=launch`

---

## Update 1 · Maker (logo após o launch, PT)

```text
Obrigado pelos upvotes e pelos comentários. Vocês acertaram o ponto: o Resolva Jato nasceu pro fluxo real do profissional brasileiro.

Hoje o núcleo gratuito inclui:
• Orçamento com aprovação e Pix no WhatsApp
• Currículo, recibo, contrato e proposta em PDF
• Calculadoras de rescisão, preço freelancer e MEI vs CLT

Estamos ouvindo tudo. Se você é MEI ou freelancer, me diga qual ferramenta quer ver aprimorada primeiro.
```

## Update 2 · Feature spotlight · Orçamento + Pix (EN, hunters)

```text
Quick spotlight: Budget with Pix on WhatsApp.

Your client opens the budget on their phone, approves, and pays with Pix.
No app install. No card. Built for how Brazilian freelancers and MEIs actually close deals.

Try it: https://resolvajato.com.br/orcamento-com-pix?utm_source=producthunt&utm_medium=referral&utm_campaign=ph_orcamento
```

## Update 3 · Feature spotlight · Currículo (EN)

```text
Need a clean resume PDF fast?

Resolva Jato gives you live preview layouts and export without wrestling with Word.
Free to start: https://resolvajato.com.br/gerador-de-curriculo?utm_source=producthunt&utm_medium=referral&utm_campaign=ph_curriculo
```

## Update 4 · Feature spotlight · Recibo + Proposta (PT)

```text
Dois geradores que o pessoal pediu bastante nos comentários:

1) Recibo em PDF com valor por extenso
https://resolvajato.com.br/gerador-de-recibo?utm_source=producthunt&utm_medium=referral&utm_campaign=ph_recibo

2) Proposta comercial com cara de agência
https://resolvajato.com.br/gerador-de-proposta-comercial?utm_source=producthunt&utm_medium=referral&utm_campaign=ph_proposta

Conta grátis pra salvar e baixar. Sem cartão.
```

## Update 5 · Social proof / pedido de feedback (mistura PT+EN)

```text
Loving the early feedback about WhatsApp + Pix for MEIs.

If you tried the budget flow or any PDF tool today, drop one honest note:
what worked, what felt confusing, what you want next.

We're building this for real Brazilian workflows, not generic SaaS theater.
```

---

## Cadência sugerida (próximos 5 dias no PH)

| Dia | Ação |
|-----|------|
| Hoje | Trocar/enriquecer gallery com as 5 imagens + thumb |
| +1 | Update 2 (Orçamento + Pix) com `ph-gallery-orcamento-pix.png` |
| +2 | Update 3 (Currículo) |
| +3 | Update 4 (Recibo + Proposta) |
| +4 | Update 5 + responder threads de MEI/freelancer |
| +5 | Post curto no Maker comment pedindo 1 feature votada |

Não invente métricas. Use só feedback real (como o dos comentários do Caleb e da Sharmistha).

---

## Respostas rápidas a hunters (templates)

**Sobre Pix + WhatsApp**
```text
Esse era o norte desde o dia 1: fechar no WhatsApp e receber no Pix, sem forçar o cliente a baixar app.
```

**Sobre curadoria enxuta**
```text
Preferimos poucas ferramentas bem resolvidas do que um menu infinito. Se sentir falta de alguma, manda a prioridade.
```

**Sobre roadmap**
```text
Estamos priorizando o que MEI e freelancer usam toda semana. Sua sugestão entra na fila com peso real.
```
